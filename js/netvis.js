...
define(["lib/d3"], function(d3) {
  ...
  visDefaults.DEF_NODE_SIZE    = 3.4;
  visDefaults.HLT_NODE_SIZE    = 3;
  visDefaults.SEL_NODE_SIZE    = 5;
  visDefaults.DEF_NODE_OPACITY = 0.9;
  visDefaults.HLT_NODE_OPACITY = 1.0;
  visDefaults.SEL_NODE_OPACITY = 1.0;

  // Function to draw labels around circle
	function drawLabels(network) {
	
    var arc = d3.svg.arc()
      .outerRadius(network.display.radius - 85)
      .innerRadius(network.display.radius - 105);

    var pie = d3.layout.pie()
      .sort(null)
      .value(function(d) { return d.counts; });

  svgarc = network.display.svg.append("g")
    .attr("transform", "translate(" + network.display.width / 2 + "," + network.display.height / 2 + ")");
  ...
  /*
   * Takes a network created by the matricesToNetwork 
