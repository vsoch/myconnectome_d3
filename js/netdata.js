 ...
 */
 ...
 define(["lib/d3", "lib/queue"], function(d3, queue) {
  ...
  function genColourScales(network, scaleInfo) {
	...
	// This maps the node.nodeData to the color scale
	// so we get a color for each group
	  return node.color;
	    //scaleInfo.nodeColourScale(
  ...
  function extractSubNetwork(network, rootIdx) {
    ...
    for (var i = 0; i < subnet.nodes.length; i++) {
      ...
      if (subnet.thumbUrl !== null) {
        var imgurl = network.thumbUrl + "/roi" + zerofmt(nodeIdxs[i]+1) + ".png";
  ...
  function thresholdNetwork(network) {
    ...
    for (var i = 0; i < numNodes; i++) {
      for (var j = i+1; j < numNodes; j++) {
        ...
        // THIS IS IMPORTANT - here we are saving the edges (based on looking
		// at distances in the correlation matrices TO the nodes themselves,
		// SO we should be able to change around the order of the network.nodes
		// and not worry about losing these relationships!
  ...
  function createNetwork(
    ...
    for (var i = 0; i < numNodes; i++) {
      ...
      // The node index refers to where we find it in the matrices
	  // as well as the index into network.metaData
	  node.index      = i;
      // The ordering is how we want to order the nodes
	  node.order      = parseInt(metaData[i]["lateral_groups"]);
      // The NAME corresponds with the image number, etc, which is +1 from 
	  // the javascript index (which starts at 0) 
      // FOR DEBUGGING PURPOSES - setting node name to be network group	  
      //node.name       = "" + (i+1);
      node.name = "";
	  //node.name       = metaData[i]["network"]; 
	  node.nodeData   = nodeData.map(function(array) {return array[i];});
      node.hemisphere = metaData[i]["hemis"];
	  node.color      = metaData[i]["color"];
	  
      // Attach a thumbnail URL to 
      // every node in the network
      if (thumbUrl !== null) {
        var imgUrl = thumbUrl + "/roi" + zerofmt(i+1) + ".png";
        node.thumbnail = imgUrl;
      }
      else {
        node.thumbnail = null;
  ...
  function onDataLoad(error, args) {
	...
	if (metaData !== null)
      metaData = parseStringMatrix(metaData);
   ...
   *
   *   - nodeDataLabels:  Optional. A list of labels for each of 
   *                     the above arrays.
   *
   ...
   * 
   *   - metaData:       A tab separated file with meta data to be
   *                     added to the nodes.  There should be a header
   *                     with variable names, and the rows should correspone
   *                     to the rows in the correlation matrix, and order
   *                     in the nodeDataLabels.  (Added by vsochat 11/10/14)
   */

   // This function is called BEFORE the function above, and parses
   // the original input args in main.js into a new args set,
   // basically making sure things are the right size, length, etc.
   function loadNetwork(args, onLoadFunc) {
	...
	// metaData
	a.metaData.forEach(function(url) {
      q = q.defer(d3.text, url);
    });
  ...
  /*
   * Let's try to parse a data matrix with strings
   */
  function parseStringMatrix(matrixText) { 
    // create a parser for tab delimited text
    var parser = d3.dsv("\t", "text/plain");
      
    // Parse the strings into a matrix
	// the function "parse" assumes the first row is header fields
	var matrix = parser.parse(matrixText, function(row) {
      return row;
    });
    return matrix;
